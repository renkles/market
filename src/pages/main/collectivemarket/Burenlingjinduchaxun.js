...
import {Button, DateRangePicker, Loading, Pagination} from 'element-react'
import _ from 'lodash'
import Http from '../../../utils/http'

import './burenlingjinduchaxun.scss'
...
class Burenlingjinduchaxun extends Component {
    ...
    getList() {
        ...
        if (params.startTime != null) {
            ...
            Http.post('/supply/progress/query',params)
                .then(
                    res=> {
                        console.log(res)
                        if(res.resultCode === 'success'){
                            let data = res.data.supplyProgressList
    ...
    formatStatus(str){
        switch(parseInt(str)) {
            case 0:
                return {label:'补认领中',color:'#F1C500'}
            case 1:
                        ...
                        <tbody>
                        {
                            ...
                            this.state.list.map((el,index) =>
                                <tr key={index}>
                                    ...
                                    <td>{`${el.supplyDate.slice(0,4)}-${el.supplyDate.slice(4,6)}-${el.supplyDate.slice(6,8)}`}</td>
                                    <td><Button size='mini' style={{backgroundColor:`${this.formatStatus(el.supplyStatus).color}`,color:'#FFFFFF'}}>{this.formatStatus(el.supplyStatus).label}</Button></td>
                                </tr>
                            )
                            :
