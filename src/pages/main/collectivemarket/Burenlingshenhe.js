...
import './burenlingshenhe.scss'
import '../../../iconfont/iconfont.css'

class Burenlingjinduchaxun extends Component {
    constructor() {
        super()
        this.state = {
            loadingShow:false,
    ...
    getList() {
        ...
        Http.post('/supply/approve/query',params)
            .then(
                res=>{
                    console.log(res)
                    if(res.resultCode === 'success'){
                        let list = res.data.supplyApproveList
                        let total = res.data.count
                        this.setState({tableList:list,total:total,supplyIdList:[],loadingShow:false})
                    }else{
                        console.log('获取数据失败')
        ...
        if(this.state.tableList != null && this.state.tableList.length > 0){
            if(val){
                ...
                _.forEach(this.state.tableList,(obj)=>{
                    list.push(obj.supplyId)
                })
                this.setState({supplyIdList:list})
            }else{
                this.setState({supplyIdList:[]})
            }
        }else{
            return
        }
    ...
    selectItem(id,val) {
        if(val){
            let list = [...this.state.supplyIdList]
            list.push(id)
            this.setState({supplyIdList:list})
        }else{
            let list = [...this.state.supplyIdList]
    ...
    showResolve(id) {
        if(typeof id === 'number'){
            this.setState({currentSupplyId:id,dialogType:3,canConfirm:true})
        }else{
            if(this.state.supplyIdList.length > 0){
    ...
    showReject(id) {
        if(typeof id === 'number'){
            this.setState({currentSupplyId:id,dialogType:4,canConfirm:true})
        }else{
            if(this.state.supplyIdList.length > 0){
    ...
    closeDialog() {
        ...
        switch (type){
            case 3:
                this.setState({currentSupplyId:"",dialogType:0,advice:"",canConfirm:false})
                break;
            case 4:
                this.setState({currentSupplyId:"",dialogType:0,advice:"",canConfirm:false})
                break;
            ...
            switch (this.state.dialogType){
                case 1: // 批量通过
                    let params1 = {
                        ...
                        status: "1",
                        supplyId: this.state.supplyIdList
                    }
                    Http.post('/supply/approve',params1)
                        .then(
                            res=>{
                                console.log(res)
                                if(res.resultCode  === 'success'){
                                    this.setState(
                                        {dialogType:0,advice:"",currentSupplyId:"",supplyIdList:[]},
                                        ()=>{
                                            MessageBox.msgbox({
                                                title:'提示',
                                                message:'审核成功',
                ...
                case 3: // 通过
                    let params2 = {
                        advice: this.state.advice,
                        status: "1",
                        supplyId: [this.state.currentSupplyId]
                    }
                    Http.post('/supply/approve',params2)
                        .then(
                            res=>{
                                console.log(res)
                                if(res.resultCode  === 'success'){
        ...
        return (
            <div className="check-clock">
                <Loading loading={this.state.loadingShow} text='加载中...'>
                    <div className="check-title">补认领审核进度列表</div>
                    <div className="table-box">
                        <table className="table-style">
                            <thead>
                            <tr>
                                <td><Checkbox onChange={this.selectAll.bind(this)} checked={this.state.tableList.length > 0 ? this.state.tableList.length === this.state.supplyIdList.length : false}/>全选</td>
                                <td>客户名称</td>
                                <td>证件号</td>
                                <td>业务类型</td>
                                <td>金额</td>
                            ...
                            <tbody>
                            {
                                this.state.tableList != null && this.state.tableList.length > 0
                                    ...
                                    this.state.tableList.map((el,index)=>
                                        <tr key={index}>
                                            <td><Checkbox onChange={this.selectItem.bind(this,el.supplyId)} checked={_.indexOf(this.state.supplyIdList,el.supplyId) !== -1}/></td>
                                            ...
                                            <td>{el.staffName}</td>
                                            <td>
                                                <i className='iconfont icon-queren' style={{margin:'0 5px',color:'#63BF6A',cursor:'pointer'}} onClick={this.showResolve.bind(this,el.supplyId)}></i>
                                                <i className='iconfont icon-chacha' style={{margin:'0 5px',color:'#D06052',cursor:'pointer'}} onClick={this.showReject.bind(this,el.supplyId)}></i>
                                            </td>
                                        </tr>
                                    )
                                    :
...
export default withRouter(Burenlingjinduchaxun)