...
import { Pagination, Loading, MessageBox, Button, Dialog, Tag} from 'element-react';
import Format from '../../../utils/format'
import Http from '../../../utils/http'
import _ from 'lodash'
import HandsonTable from '../../../components/handsonTable/HandsonTable'

import './wowanchenguode.scss'
import '../../../iconfont/iconfont.css'

class Wowanchenguode extends Component{
    constructor() {
        super()
        this.state={
            loading:false,
    ...
    getList(){
        ...
        Http.post('/task/myTask/history/list',params)
            .then(res=>{
                console.log(res)
    ...
    showDetail(id) {
        ...
        Http.post('task/myTaskInfo/query', params)
            .then(res=>{
                console.log(res)
                if(res.resultCode === 'success'){
        ...
        return(
            <div style={{position:'relative'}}>
                <div className='approved-list-block' style={parseInt(this.state.currentPageType) === 1 ? {} : {transform:'scale(0)'}}>
                    <div className="approve-list-title">已完成任务</div>
                                    ...
                                    this.state.list.map((el,index)=>
                                        <li onClick={this.showDetail.bind(this, el.id)} className="approved-item" key={index}>
                                            ...
                                            <div className='task-item-info'>
                                                ...
                                                <br/>
                                                <span style={{color:'#999999'}}>完成时间：{new Date(el.finishedTime).toLocaleString().replace(/\//g,'-')}</span>
                                            </div>
                                            <div className="task-status-icon">
                                                <i style={{fontSize:'18px',lineHeight:'60px',color: "#54D092"}} className="el-icon-circle-check"></i>
                                            </div>
                                        </li>
                                    )
                                    :
...
export default withRouter(Wowanchenguode)